...
@Component({
  selector: 'app-bophanbaophe',
  templateUrl: './bophanbaophe.component.html',
  styleUrls: ['./bophanbaophe.component.css']
})
export class BophanbaopheComponent implements OnInit {
  ...
  constructor(private cmpDiagramService: CompanyDiagramService, private baopheService: BaopheService, private dg: ModalService) { }
  mPBMapper: boolean;
  mDMBaoPhe: boolean;
  A0034_ID: string;
  checkLen: number;
  ...
  getListphongBanMapper() {
    this.Temp = [];
    this.baopheService.GetListPhongBanMapper(this.opition.pz, this.opition.p, this.opition.sort, this.opition.ob, this.opition.s, this.opition.sts, 2).subscribe((data: Array<object>) => {
      var pb = data["data"];
  ...
  modalPhongBanMapper(isType: string) {
    ...
    this.PBMapper = { Parent_ID: null, phongBanMapID: null, STT: this.ListPBMapper.length + 1, kieuPhongBan: 2, trangThai: true };
