...
@Component({
  selector: 'app-congviectrinhky',
  templateUrl: './congviectrinhky.component.html',
  styleUrls: ['./congviectrinhky.component.css']
})
export class CongviectrinhkyComponent implements OnInit {
  ...
  getListMyDocument() {
    ...
    this.workService.MyWaitingForSign(this.opition.pz, this.opition.p, this.opition.sort, this.opition.ob, this.opition.s, this.opition.sts, Users_ID).subscribe((data: Array<object>) => {
  ...
  RejectDocument() {
    var tk = JSON.parse(localStorage.getItem("login"));
    var Users_ID = tk["Users_ID"];
    this.workService.RejectDocument(this.rowSelect, Users_ID,this.DocReject["noiDung"]).subscribe((then) => {
      this.getListMyDocument();
      this.hideModal();
    });
  } 
