...
@Component({
  selector: 'app-congviecdaxoa',
  templateUrl: './congviecdaxoa.component.html',
  styleUrls: ['./congviecdaxoa.component.css']
})
export class CongviecdaxoaComponent implements OnInit {
  ...
  getListMyDocument() {
    ...
    this.workService.TrashDocument(this.opition.pz, this.opition.p, this.opition.sort, this.opition.ob, this.opition.s, this.opition.sts, Users_ID).subscribe((data: Array<object>) => {
  ...
  deleteList() {
    this.dg.confirm('Confirm', 'Bạn có chắc chắn muốn xóa mục đã chọn ?')
      .then((confirmed) => {
        if (confirmed) {
          this.workService.DeleteDocumentTrash(this.rowSelect).subscribe((then) => {
            if (then["error"] == 0) {
              this.getListMyDocument();
            }
          });
