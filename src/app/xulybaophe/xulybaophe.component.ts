...
@Component({
  selector: 'app-xulybaophe',
  templateUrl: './xulybaophe.component.html',
  styleUrls: ['./xulybaophe.component.css']
})
export class XulybaopheComponent implements OnInit {
  ...
  getListMyDocumentCam() {
    ...
    this.workCAMService.MyWaitingConFirmBaoPhe(this.opition.pz, this.opition.p, this.opition.sort, this.opition.ob, this.opition.s, this.opition.sts, Users_ID).subscribe((data: Array<object>) => {
  ...
  UpdateSF006() {
    ...
    formData.append("a0037", JSON.stringify(this.rowHS));
    formData.append("a0028D", JSON.stringify(ListD));
    this.workCAMService.updateWorkDocumentCamConfirm(formData).subscribe(
      (then) => {
        this.hideModalForm();
        this.getListMyDocumentCam();
      }
